...
// --- Grid Thumbnail Card ---
const FeelGridCard: React.FC<{
    feel: FeelVideo;
    onOpen: () => void;
    onLike: () => void;
}> = React.memo(({ feel, onOpen, onLike }) => {
    const { showSuccess } = useToast();
    const [isLiked, setIsLiked] = useState(false);
    const [likes, setLikes] = useState(feel.likes || 0);

    const videoId = useMemo(() => getYouTubeId(feel.videoUrl), [feel.videoUrl]);
    ...
    const authorName = useMemo(() => feel.user.fullName || feel.user.email.split('@')[0], [feel.user]);

    useEffect(() => { setLikes(feel.likes || 0); }, [feel.likes]);
    ...
    const handleShare = useCallback((e: React.MouseEvent) => {
        e.stopPropagation();
        if (navigator.share) {
            navigator.share({ title: feel.caption || 'Check out this Feel', url: window.location.href }).catch(() => {});
        } else {
            navigator.clipboard.writeText(window.location.href);
            showSuccess('Link copied!');
        }
    }, [feel.caption, showSuccess]);
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-2 left-2 flex items-center gap-1.5">
                <img
                    src={feel.user.avatarUrl || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=64'}
                    alt={authorName}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2.5">
                {feel.caption && (
                    <p className="text-[10px] text-white/80 line-clamp-1 mb-1.5">{feel.caption}</p>
                )}
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        <span className="text-[10px] font-semibold text-white drop-shadow">{formatCount(feel.shares || 0)}</span>
                    </button>
                </div>
            </div>
        </div>
...
FeelGridCard.displayName = 'FeelGridCard';
...
const VideoLightbox: React.FC<{
    ...
    hasPrev: boolean;
    hasNext: boolean;
}> = ({ feel, onClose, onLike, onPrev, onNext, hasPrev, hasNext }) => {
    const { showSuccess } = useToast();
    const [isLiked, setIsLiked] = useState(false);
    ...
    const videoId = useMemo(() => feel ? getYouTubeId(feel.videoUrl) : null, [feel]);
    const isLocalVideo = feel && !videoId && feel.videoUrl;
    const authorName = useMemo(() => feel ? (feel.user.fullName || feel.user.email.split('@')[0]) : '', [feel]);

    useEffect(() => {
        if (feel) {
            setIsLiked(false);
            setLikes(feel.likes || 0);
            setIsPlaying(false);
        }
    }, [feel?.id]);
                ...
                <div className="w-full sm:w-72 flex flex-col bg-gray-950 border-t sm:border-t-0 sm:border-l border-white/10">
                    ...
                    <div className="flex items-center gap-3 p-4 border-b border-white/10">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-xs text-white/50 truncate">{feel.user.email.split('@')[0]}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-4 overflow-y-auto">
                        {feel.caption && (
                            <p className="text-sm text-white/80 leading-relaxed">{feel.caption}</p>
                        )}
                        {feel.music && (
                            <p className="text-xs text-brand-primary/70 mt-2 flex items-center gap-1">
                                <span>♪</span> {feel.music}
                            </p>
                        )}
                    </div>
...
}> = React.memo(({ isOpen, onClose, isLoading, error, recommendations }) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm">
            <div className="relative bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-2xl shadow-2xl w-full max-w-3xl p-4 sm:p-6 flex flex-col max-h-[90vh]">
                <button
                    onClick={onClose}
                    className="absolute top-3 right-3 sm:top-4 sm:right-4 p-1 text-light-text-secondary dark:text-dark-text-secondary hover:text-light-text-primary dark:hover:text-dark-text-primary"
                    aria-label="Close"
                >
                    <CloseIcon className="w-5 h-5 sm:w-6 sm:h-6" />
                </button>
                <h2 className="text-xl sm:text-2xl font-bold text-center text-light-text-primary dark:text-dark-text-primary mb-4 sm:mb-6">✨ Your Next Favorite 'Feels'</h2>
...
RecommendationsModal.displayName = 'RecommendationsModal';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-3 py-4 sm:p-4 bg-black/70 backdrop-blur-sm">
            <div className="relative bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-2xl shadow-2xl w-full max-w-lg p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-4 sm:mb-5">
                    <h2 className="text-xl sm:text-2xl font-bold text-light-text-primary dark:text-dark-text-primary">Upload Feels Video</h2>
                    <p className="mt-1 text-xs sm:text-sm text-light-text-secondary dark:text-dark-text-secondary">Share your real estate moments with the community</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            {!videoFile ? (
                                <div onDrop={handleDrop} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={e => { e.preventDefault(); setDragOver(false); }}
                                    ...
                                    className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-all ${dragOver ? 'border-brand-primary bg-brand-primary/10' : 'border-light-border dark:border-dark-border hover:border-brand-primary/50'}`}
                                >
                    ...
                    <div>
                        ...
                        <textarea value={caption} onChange={e => setCaption(e.target.value)} placeholder="What's happening in this video?" rows={2}
                    ...
                    <div>
                        <label className="block text-xs sm:text-sm font-semibold text-light-text-primary dark:text-dark-text-primary mb-1.5">Music / Audio Credit</label>
                        <input type="text" value={music} onChange={e => setMusic(e.target.value)} placeholder="e.g., Original Audio - @username"
                            className="w-full bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-lg px-3 py-2.5 text-xs sm:text-sm text-light-text-primary dark:text-dark-text-primary focus:ring-2 focus:ring-brand-primary focus:outline-none" />
                    </div>
...
// --- Main Page ---
const FeelsPage: React.FC<FeelsPageProps> = ({ isAuthenticated, isAdmin, currentUser }) => {
    const [videos, setVideos] = useState<FeelVideo[]>([]);
    ...
    const loadVideos = useCallback(async () => {
        try {
            ...
            if (response.success) setVideos(Array.isArray(response.data) ? response.data : []);
            else setVideos([]);
        } catch { setVideos([]); }
    ...
    const handleLikeVideo = useCallback(async (videoId: string) => {
        try { await feelsAPI.like(videoId); } catch {}
        // optimistic — don't reload full list
    }, []);

    const handleUpload = useCallback(async (data: { videoUrl: string; caption?: string; music?: string }) => {
        setUploading(true);
        try {
            const response = await feelsAPI.create(data);
            if (response.success) {
        ...
        <div className="min-h-screen bg-light-bg dark:bg-dark-bg">
            ...
            <div className="relative h-44 sm:h-56 md:h-64 rounded-2xl overflow-hidden mb-6 sm:mb-8 shadow-lg">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col items-center justify-center p-4 text-center">
                    ...
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-white via-purple-100 to-brand-primary/80 drop-shadow-lg">
                        ShelTrify Feels
                    </h1>
                    <p className="mt-1 text-sm text-white/75 max-w-xs">Your daily dose of real estate inspiration.</p>
                    {isAuthenticated && (
                        <div className="mt-3 flex flex-wrap justify-center gap-2">
                            ...
                            <button onClick={handleGetRecommendations} disabled={isFetchingRecs}
